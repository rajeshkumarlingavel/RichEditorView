...
@objcMembers open class RichEditorView: UIView, UIScrollViewDelegate, UIWebViewDelegate, UIGestureRecognizerDelegate {
    ...
    public var isWhiteTheme:Bool = false{
        didSet{
            
            var  fileName = isWhiteTheme ? "rich_editorwhite" : "rich_editor"
            if isEditingEnabled{
                fileName = fileName + "_reply"
            }
            if let filePath = Bundle(for: RichEditorView.self).path(forResource: fileName, ofType: "html") {
...
//        let fileName = isWhiteTheme ? "rich_editorwhite" : "rich_editor"
//        if let filePath = Bundle(for: RichEditorView.self).path(forResource: fileName, ofType: "html") {
//            let url = URL(fileURLWithPath: filePath, isDirectory: false)
//            let request = URLRequest(url: url)
//            webView.loadRequest(request)
...
//                    NSLog("RichEditorView: Failed to parse JSON Commands")
        ...
        if let components = request.url?.absoluteString.components(separatedBy: ":")[0],components == "radarmorekeypressed"{
            delegate?.richEditor?(self, handle: "radarmorekeypressed")
        }
    ...
    private func performCommand(_ method: String) {
        if method.hasPrefix("ready") {
            let css = "".getCss(textColor: UIColor.red, bgColor: UIColor.green)
            let js = "var style = document.createElement('style'); style.innerHTML = '\(css)'; document.head.appendChild(style);alert(okay);location.reload();"
//            runJS(js)
            webView.stringByEvaluatingJavaScript(from: "alert(okay);")//evaluateJavaScript(js, completionHandler: nil)
...
extension String{
    func getCss(textColor:UIColor,bgColor:UIColor) -> String{
        return """
